...
class Ipl:
    ...
    def get_data(self, url, tag):
        ...
        for x in range(1, 25, 2):
            time.sleep(5)
            match_info = {'data1': [], 'data2': [], 'scorecard': []}
            ...
            try:
                ...
                for j in range(1, 20):
                    try:
                        data1 = team_info.find_element(By.XPATH, f'/html/body/div/section/section/div[5]/div[1]/div['
                                                                 f'3]/div/div/table/tbody/tr[{x+1}]/td/div/div/div'
                                                                 f'/div/a[{j}]/div/div[2]/span[1]')
                        data2 = team_info.find_element(By.XPATH, f'/html/body/div/section/section/div[5]/div[1]/div['
                                                                 f'3]/div/div/table/tbody/tr[{x+1}]/td/div/div/div'
                                                                 f'/div/a[{j}]/div/div[2]/span[2]')
                        scorecard_link = team_info.find_element(By.XPATH, f'/html/body/div/section/section/div['
                                                                          f'5]/div[1]/div[3]/div/div/table/tbody/tr['
                                                                          f'2]/td/div/div/div/div/a[{j}]')
                        match_info['data1'].append(data1.text)
                        match_info['data2'].append(data2.text)
                        match_info['scorecard'].append(scorecard_link.get_attribute('href'))
                    except NoSuchElementException:
                        break
                df2 = pd.DataFrame(match_info)
                df2.to_csv(f'../data/match data {tag}/{team_name.text} {tag}.csv', index=False)
    ...
    def save_data(self, tag):
        ...
        df.to_csv(f'../data/points table/data{tag}.csv', index=False)
